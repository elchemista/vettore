...
// Simple embedding with ID, vector, metadata
#[derive(Debug, Clone)]
...
pub struct DBResource(Mutex<CacheDB>);
...
impl CacheDB {
    ...
    pub fn insert_embedding(
        ...
        // If distance is Cosine, normalize
    ...
    pub fn get_embedding_by_id(
        ...
        coll.embeddings
            .iter()
            .find(|emb| emb.id == id)
            .cloned() // clone the found Embedding
    ...
    pub fn similarity_search(
        ...
        let mut scored: Vec<(Embedding, f32)> = coll.embeddings
            .iter()
            .map(|emb| {
        ...
        // Sort in ascending order for Euclidean (lowest distance is "most similar")
        // Sort descending for Cosine or Dot (largest dot is "most similar")
        match coll.distance {
            Distance::Euclidean => {
...
fn get_embedding_by_id(
    ...
    // Return the embedding fields you want; here, (id, vector, metadata)
