defmodule Vettore.Collection do
  ...
  alias Vettore.{Distance, Embedding, Result}
  ...
  @doc """
  Saves the collection ETS table to a snapshot file.

  Only canonical ETS state is written. Native index state is rebuilt when the
  snapshot is loaded.

  ## Examples

      iex> {:ok, collection} = Vettore.Collection.new(dimensions: 2, metric: :l2)
      iex> path = Path.join(System.tmp_dir!(), "vettore_snapshot_example.ets")
      iex> Vettore.Collection.snapshot(collection, path)
      :ok
      iex> File.rm(path)
      :ok
  """
  ...
  @doc """
  Loads a collection from a snapshot file.

  The saved collection config is reused by default. Pass options such as
  `index: :flat` or `index: :hnsw` to rebuild the loaded collection with a
  different index.

  ## Examples

      iex> {:ok, collection} = Vettore.Collection.new(dimensions: 2, metric: :l2)
      iex> :ok = Vettore.Collection.put(collection, %{id: "a", vector: [1.0, 2.0]})
      iex> path = Path.join(System.tmp_dir!(), "vettore_load_example.ets")
      iex> :ok = Vettore.Collection.snapshot(collection, path)
      iex> {:ok, loaded} = Vettore.Collection.load_snapshot(path)
      iex> {:ok, embedding} = Vettore.Collection.get(loaded, "a")
      iex> File.rm(path)
      iex> embedding.id
      "a"
  """
  ...
  @doc """
  Runs Matryoshka-style funnel search over progressively larger vector prefixes.

  The configured store remains canonical. Each stage scores a candidate set with
  a vector prefix, then the final results are reranked with the full stored
  vectors.
  """
    ...
    with :ok <- validate_limit(limit),
      ...
      stage_embeddings =
        Enum.reduce(stages, embeddings, fn dimensions, acc ->
          collection
          |> score_embeddings(acc, take_dimensions(query, dimensions), candidates, dimensions)
  ...
  The first pass compresses query and stored vectors to sign bits and ranks by
  Hamming distance. Final results are reranked with the collection metric over
  the full stored vectors.
  """
  ...
  @doc """
  Validates and normalizes a query for the collection.
  """
